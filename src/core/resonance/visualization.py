...
class Visualizer:
    ...
    def plot_day_analysis(
        ...
        """Plot analysis results for a single day using power spectrum only."""
        plt.figure(figsize=(10, 6))
        
        # Plot power spectrum only
        plt.plot(
        ...
        for freq, power in zip(peak_freqs, signal_data.power_spectrum[peaks]):
            plt.text(freq, power, f'{freq:.1f}', color='red', ha='center', va='bottom')
        
        # Add element bands
        for el, bands in self.config.get_harmonic_frequencies().items():
            for b in bands:
                plt.axvline(b, color='r', linestyle='--', alpha=0.5, label=f'{el} reference')
        
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Power')
        plt.title(f'{day} Power Spectrum - Patient {patient_id}')
        plt.legend()
    ...
    def plot_day_comparison(
        ...
        """Plot comparison between Day 1 and Day 5 using power spectra."""
        plt.figure(figsize=(10, 6))
        
        if day1_data is not None and day1_data.frequencies.size:
            plt.plot(
        ...
        if day5_data is not None and day5_data.frequencies.size:
            plt.plot(
        ...
        for el, bands in self.config.get_harmonic_frequencies().items():
            for b in bands:
                plt.axvline(b, color='gray', linestyle='--', alpha=0.5, label=f'{el} reference')
